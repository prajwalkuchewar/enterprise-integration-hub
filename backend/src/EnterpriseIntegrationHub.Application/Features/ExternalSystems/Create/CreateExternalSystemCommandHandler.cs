...
public sealed class CreateExternalSystemHandler
{
  ...
  public async Task<Guid> Handle(CreateExternalSystemCommand command, CancellationToken cancellationToken)
  {
    var exists = await _repository.ExistsAsync(command.Name, command.Environment, cancellationToken);
